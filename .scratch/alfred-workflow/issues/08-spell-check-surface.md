...
The measurement is in [the probe](../prototypes/08-spell-check/probe.md) and reproducible from the three scripts beside it. It reframed the ticket before answering it.

### The premise was wrong

This ticket asked whether a step firing "on nearly one title in five" earns its place. It does not fire at that rate. The 18 per cent in `spell_check.py`'s docstring is a **token-level** rate from a synthetic technical vocabulary; read as a per-title rate it overstates the problem by a factor of fifty.
...
The check costs **0.002 ms**. Its price is measured only in keystrokes. That makes the shape decision, not the keep-or-drop decision, the one that matters — and it points at a shape that costs no keystrokes at all.
...
A dedicated step is ruled out on the numbers: seen once in seventy creations, in a surface whose whole point is speed, and offering a **wrong** word 25 per cent of the times it appears.
